...
const CalendarPopup = ({onClose}) => (
    <div className={styles.popupContainer} onClick={onClose}>
        ...
        <span className={styles.popupClick} onClick={() => {
            alert('질병 캘린더로 이동합니다.');
        }}>
            질병 캘린더로 이동하기
        </span>
                ...
                {diseaseList.map(disease => (
                    <div key={disease.id} className={styles.like_item_wrapper}>
                        ...
                        <div
                            ...
                            className={`${isSelectionMode ? styles.like_item_container_delete : styles.like_item_container} ${clickedId === disease.id ? styles.clickedItem : ''}`}
                        >
                            <div className={`${styles.title} ${clickedId === disease.id ? styles.clickedItem : ''}`}>
                                직짱인12345님이 {disease.date}에 한 건강 설문입니다.
                            </div>
                            <div className={`${styles.date} ${clickedId === disease.id ? styles.clickedItem : ''}`}>
                                <Image src={calendarIcon} alt={'calendar'} />
                                {disease.isLinked ? '질병 캘린더에 연동됨' : '질병 캘린더 연동중'}
                            </div>
                            <div className={`${styles.title} ${clickedId === disease.id ? styles.clickedItem : ''}`}>
                                <span
                                    className={`${styles.place} ${clickedId === disease.id ? styles.clickedItem : ''}`}>{disease.diseasePath}</span>
                            </div>
                        </div>
                    </div>
                ))}
                {isSelectionMode && (
                    <button className={styles.chice_delete} onClick={handleDeleteSelected}>선택한 질병 리스트 삭제</button>
                )}
            </div>
            {showPopup && activeDiseaseId && (
